...
RAW_INPUT_MAPPING_PATH = ROOT / "config" / "raw_input_mapping.yaml"

# Fingerprint: tập cột bắt buộc phân biệt từng bảng
_TABLE_COLUMN_FINGERPRINTS: dict[str, set[str]] = {
...
def detect_table_from_columns(columns: list[str]) -> str | None:
    """Trả về tên bảng nếu cột khớp fingerprint, ngược lại None."""
    col_set = {c.strip().lower() for c in columns}
    best_table: str | None = None
    best_score = 0
    for table, required in _TABLE_COLUMN_FINGERPRINTS.items():
...
def update_raw_input_mapping(table_name: str, file_path: Path) -> None:
    """Ghi/cập nhật mapping table_name → file_path vào raw_input_mapping.yaml."""
    RAW_INPUT_MAPPING_PATH.parent.mkdir(parents=True, exist_ok=True)
    if RAW_INPUT_MAPPING_PATH.exists():
        payload: dict = yaml.safe_load(RAW_INPUT_MAPPING_PATH.read_text(encoding="utf-8")) or {}
    else:
        payload = {}
    payload[table_name] = str(file_path.relative_to(ROOT))
    RAW_INPUT_MAPPING_PATH.write_text(
...
class ValidationResult:
    ...
    detected_table: str | None = field(default=None)
    ...
    try:
        if suffix == ".csv":
            ...
            return ValidationResult(
                name, False, "Định dạng file không được hỗ trợ.", 0, [], pd.DataFrame()
            )

        cols = preview.columns.tolist()
        detected = detect_table_from_columns(cols)

        if name in EXPECTED_RAW_FILES:
            message = "Hợp lệ (tên file chuẩn)"
        elif detected:
            message = f"Hợp lệ — tự nhận diện là bảng `{detected}`"
        else:
            message = "Hợp lệ nhưng không nhận diện được bảng (cột không khớp bảng nào)"
...
def save_uploaded_files(
    files: Iterable, validation_map: dict[str, ValidationResult] | None = None
) -> list[Path]:
    ...
    for file_obj in files:
        ...
        # Tự ghi mapping nếu nhận diện được bảng và tên file không phải chuẩn
        if file_obj.name not in EXPECTED_RAW_FILES:
            result = validation_map.get(file_obj.name) if validation_map else None
            if result and result.detected_table:
                update_raw_input_mapping(result.detected_table, out_path)
                logger.info("Auto-mapped %s -> %s", file_obj.name, result.detected_table)
    ...
    for script_path in notebook_paths:
        if not script_path.exists():
            ...
            if not parent.exists():
                hints.append(f"Thu muc cha '{parent}' KHONG ton tai")
            else:
                siblings = sorted(f.name for f in parent.iterdir() if f.is_file())[:10]
                hints.append(f"Files trong '{parent.name}': {siblings}")
            hints.append(
                "Kiem tra thu muc project khong bi long 2 lop "
                "(vd: Nhom_03_BigData-main/Nhom_03_BigData-main)"
            )
...
def validate_project_structure() -> dict[str, object]:
    """Kiem tra cau truc project, tra ve {ok, messages}."""
    messages: list[str] = []
    required_dirs = ["src", "web", "data", "models", "reports", "config"]
    for d in required_dirs:
        p = ROOT / d
        if not p.exists():
            messages.append(f"Thieu thu muc: {p}")

    for group in ("preprocessing", "reporting"):
        scripts = resolve_script_list(group)
        for s in scripts:
            if not s.exists():
                messages.append(f"Script '{group}' khong ton tai: {s}")

    retrain_families = [
        "classification", "regression", "clustering",
        "recommendation", "pattern_mining",
    ]
    for fam in retrain_families:
        s = resolve_retrain_script(fam)
        if s and not s.exists():
            messages.append(f"Retrain script '{fam}' khong ton tai: {s}")

    if not messages:
        messages.append("Tat ca files va thu muc can thiet deu ton tai.")
    return {"ok": len(messages) == 1 and "ton tai" in messages[0], "messages": messages, "root": str(ROOT)}
