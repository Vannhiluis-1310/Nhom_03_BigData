...
st.caption(
    "Upload file CSV/Parquet → hệ thống tự nhận diện bảng dựa trên tên cột "
    "và cập nhật mapping. Sau đó chạy preprocessing để tạo lại train/val/test."
)
...
if uploads:
    for up in uploads:
        ...
        validation_map[up.name] = result
        label = f"{up.name} | {'✅ Hợp lệ' if result.valid else '❌ Lỗi'}"
        if result.valid and result.detected_table:
            label += f" | Bảng: {result.detected_table}"
        with st.expander(label, expanded=True):
            if result.valid:
                c1, c2, c3 = st.columns(3)
                c1.metric("Trạng thái", "Hợp lệ ✅" if result.valid else "Lỗi ❌")
                c2.metric("Số dòng preview", result.rows)
                c3.metric(
                    "Nhận diện bảng",
                    result.detected_table if result.detected_table else "Không rõ",
                )
                st.markdown("**Cột trong file:**")
                st.code(", ".join(result.columns), language=None)
...
if st.button(
    ...
    saved = save_uploaded_files(valid_files, validation_map)
...
with st.expander("Project structure check", expanded=False):
    check = validate_project_structure()
    st.caption(f"ROOT: {check['root']}")
    if check["ok"]:
        st.success(check["messages"][0])
    else:
